...
class linkedList
{
    ...
    public void reverse()
    {
    	...
    	while(current != null)
    	{
    		previous = current.getLinkPrev();
    		next = current.getLinkNext();
    		current.setLinkNext(previous);
    		current.setLinkPrev(next);
    		//as previous and next are swapped we select previous
    		current = current.getLinkPrev();
    		 		
    	}
    	start = previous.getLinkPrev();
